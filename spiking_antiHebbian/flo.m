...
elseif step==1
    ...
    for j=1:para.layer
        u{j}=false([size(neuron_P{j}),(time+2*para.window)/dt]);
    end;
    neuron_I{para.layer}=beta*para.I0*(2*ext-1);
    %%%%%%%%%%%%%
    for iT=1:1:para.window/dt
        for j=1:para.layer
            neuron_V{j}(neuron_V{j}>=para.spike)=para.V_reset;
        ...
        for j=2:para.layer-1
            neuron_P{j}=min(para.I0,max(0,neuron_P{j}+para.dt./para.tau_I.*(-neuron_P{j}+weight{j-1}'*u{j-1}(:,:,iT)./para.dt+weight_Inv{j}*u{j+1}(:,:,iT)./para.dt)));
        end;
        j=para.layer;
        %         u{j}=random('poisson',act(y*para.I0)*para.dt);
        %         neuron_P{j}=beta*para.I0*y;
        neuron_P{j}=min(para.I0,max(0,neuron_P{j}+para.dt./para.tau_I.*(-neuron_P{j}+weight{j-1}'*u{j-1}(:,:,iT)./para.dt)));
    end;
    %%%%%%%%%%%%%
    for iT=para.window/dt+1:1:(time+2*para.window)/dt
        for j=1:para.layer
            neuron_V{j}(neuron_V{j}>=para.spike)=para.V_reset;
            %             neuron_V{j}=neuron_V{j}+para.dt./para.tau_V.*(para.E_L-neuron_V{j}+para.Rm.*neuron_P{j}+para.Rm*neuron_I{j});
    ...
    %%%%%%%%%%%%%
    for iT=para.window/dt+1:1:(time+para.window)/dt
        for j=1:para.layer-1
            chan{j}=chan{j}+para.A_pos(j)*u{j}(:,:,iT)*(sum(u{j+1}(:,:,iT-para.window/dt:1:iT+para.window/dt).*permute(f_STDP(-para.window:dt:para.window,para.window),[1,3,2]),3))';
        end;
        for j=1:para.layer-1
            chan_Inv{j}=chan_Inv{j}+para.A_pos(j)*(sum(u{j}(:,:,iT-para.window/dt:1:iT+para.window/dt).*permute(f_STDP(-para.window:dt:para.window,para.window),[1,3,2]),3))*u{j+1}(:,:,iT)';
        end;
    end;
end;
end