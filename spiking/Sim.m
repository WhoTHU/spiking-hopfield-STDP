...
% [neuron_P,~,~,~,pred]=flo(itr(1)*5,neuron_P,neuron_V,0,0,para,0);
...
%             s0_V{j}=neuron_V{j}(:,1+(iIm-1)*batch_size:last_ind);
        ...
        [s0_P,~,~,~,pred]=flo(itr(1),s0_P,s0_V,0,0,para,0);
        for j=2:layer
...
%             neuron_V{j}(:,1+(iIm-1)*batch_size:last_ind)=s0_V{j};
        ...
        %modify
        for j=1:layer-1
            weight{j}=max(gmin(j),min(gmax(j),(1-para.A_pos(j)*para.weight_dec)*weight{j}+para.A_pos(j)./batch_size*gmax(j)*chan{j}));
            weight_Inv{j}=max(gmin_Inv(j),min(gmax_Inv(j),(1-para.A_pos(j)*para.weight_dec)*weight_Inv{j}+para.A_pos(j)./batch_size*gmax_Inv(j)*chan_Inv{j}));
...
%             weight_Inv{j}=weight{j};
    ...
    if mod(iEpo,test_fre)==0
        [neuron_P_t,~,~,~,pred]=flo(itr(1),neuron_P_t,neuron_V_t,0,0,para,0);
        err_test(iEpo)=mean(pred-1~=test_label);
        %         figure;
        %         scatter(weight{2}(:),weight_Inv{2}(:));
        disp([err_train(iEpo), err_test(iEpo)]);
        disp([mean(act(neuron_P{1}(:))),mean(act(neuron_P{2}(:))),mean(act(neuron_P{3}(:)))]./act(para.Imax))
    end;
%    if mod(iEpo,10)==0
