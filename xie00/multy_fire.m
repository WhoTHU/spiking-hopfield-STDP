function [ weit,weit0,S_0,S_p,S_m,S,g_E,r] = multy_fire( para,weit_ini,weit0_ini,weit_p,weit_m,learn_time,learn_Intv,if_learn,dp)
...
%     r(r(:,T0/dt+1)>0.5 | r(:,T0/dt+1)<0.01,T0/dt+1)=0.2;
    for iT=T0/dt+1:T0/dt+para.Intv_p*(para.rep-1)/dt+1
        S(V(:,iT)>=para.V_th,iT)=1;
        V(V(:,iT)>=para.V_th,iT)=para.V_reset;
        g_E(:,iT)=(weit(:,:,iT)-dp*diag(diag(weit(:,:,iT))))*r(:,iT)+weit0(:,iT).*r_0(:,iT)+weit_p.*r_p(:,iT);
    ...
    r(r(:,iT+1)>0.4 | r(:,iT+1)<0.01,iT+1)=0.1;
    for iT=T0/dt+para.Intv_p*(para.rep-1)/dt+2:T0/dt+learn_Intv/dt
        S(V(:,iT)>=para.V_th,iT)=1;
        V(V(:,iT)>=para.V_th,iT)=para.V_reset;
        ...
        dW=zeros(para.num);
        if if_learn && iT-T0/dt>(para.Intv_p*(para.rep-1)+lambda+tau)/dt+1 && iT-T0/dt<learn_Intv/dt+(lambda-tau)/dt
